...
// ------------------------------------------------------------
// 정책(사용자 지시 PR #256):
//   - 이 헬퍼는 **오직 안내 문구(suggestAnalysis) 판단용**으로만 사용.
//   - Tier 1 라우팅에서 "분석 신호 → null 반환"으로 metric_lookup 을 취소하지 않는다.
//     그 이유: 사용자가 현황집계 라디오를 선택한 경우 intent=analysis 로 넘어가면
//     aggregate SQL 파이프라인이 상관분석 SQL 을 만들어 HTTP 400 회귀를 유발했기 때문.
//   - 실행 경로는 그대로 metric_lookup handler 를 유지하되, 이 헬퍼가 true 를
//     반환하면 답변 하단에 "분석질문을 이용하면 더 좋은 답변" 파란 카드를 표시.
//
// (구 PR #255 에서 이 헬퍼를 Tier 1 취소 신호로 사용했다가 aggregate + analysis
//  경로 오작동으로 회귀가 있었음. PR #256 에서 원복.)
// ============================================================
 ...
 *   - "결과 ... 알려" / "결과 ... 보여" / "결과 ... 같이"
 *
 * 용도(PR #256): handleMetricLookup 등 aggregate 경로 handler 에서
 *   suggestAnalysis 필드 세팅 여부 판단.
 *   ★ Tier 1 라우팅 취소용으로 사용하지 않음.
 *
 * @param {string} query
 * @returns {boolean}
 */
...
export function classifyConversationalIntentHeuristic(query, conversationContext) {
  ...
  //    "영업이익 산식", "ROIC 정의" 등
  //
  // [PR #256] 정책 확정 — 산식 언급이 있으면(분석 신호가 함께 있더라도)
  //   현황집계 라디오 상태의 aggregate 흐름을 유지하기 위해 metric_lookup 로 확정한다.
  //   분석 신호에 대한 대응은 "안내 문구(suggestAnalysis)"만 붙이는 방식으로,
  //   handleMetricLookup 내부의 hasAnalysisSignal 검사 → suggestAnalysis:true
  //   경로에서 처리한다. 실행 경로 자체는 절대 analysis 파이프라인으로 넘기지 않는다.
  //
  //   ★ 배경: PR #255 는 이 지점을 "분석 신호 시 null 반환 → Tier 2 위임"으로 바꿔
  //     결과적으로 aggregate + intent=analysis 조합이 되어 상관분석 SQL 이 생성되고
  //     HTTP 400 이 발생하는 회귀를 유발. 사용자 요청에 따라 원복.
  if (
...
export async function handleMetricLookup({ query, activeDomain, conversationContext, pool, openai, model }) {
  ...
  // [PR #256] 사용자 질문에 분석 신호(상관관계·추세·원인 등)가 함께 있으면
  //   현황집계에서는 실제 분석을 수행하지 않으므로 "분석 변수 요약 + 컬럼 코드 매핑 +
  //   현황집계 안내 + 산식(코드 기반)" 순서로 답변하도록 지시.
  //   실행 경로는 그대로 metric_lookup handler 유지 (SQL 실행 X, 텍스트 응답만).
  //   suggestAnalysis 필드는 buildConversationalResponse 에서 true 로 세팅되어
  //   프런트가 파란 안내 카드를 별도로 추가로 렌더링한다.
  const analysisHintRule = analysisHint
    ? `

[★ 현황집계 + 분석 요청 답변 형식 — 매우 중요 (PR #256)]
사용자 질문에는 지표 산식뿐 아니라 **상관관계·추세·원인 분석 등 실제 데이터 분석 요청**이
함께 들어 있습니다. 현재 응답 경로는 **현황집계**이며, 이 경로는 산식을 정리해 알려줄 뿐
실제 상관계수 계산·데이터 집계·추이 도출은 수행하지 않습니다.

다음 형식과 순서를 따라 답변하세요:

  1) **분석 변수 요약**: 사용자가 언급한 지표들을 한 문장으로 정리
     예: "${activeDomain} 기준 분석 변수는 거래처별 영업이익률과 지급수수료(변동) 원단위입니다."

  2) **컬럼 코드 매핑**: 각 지표가 실제 사용하는 컬럼 코드를 한 줄로 나열
     예: "영업이익은 ZAMT055, 순매출은 ZAMT003, 지급수수료(변동)는 ZAMT040입니다."
     (조회된 formula 에서 등장하는 컬럼 코드만 사용. 임의 코드 생성 절대 금지.)

  3) **현황집계 안내**: 아래 문장을 자연스럽게 포함
     "현재 현황집계에서는 실제 거래처별 상관관계 분석을 수행하지 않았습니다."

  4) **산식**: 코드 블록 대신 한 줄씩 나열 (사용자 지정 산식이 있으면 그대로 반영)
     예:
       \`영업이익률 = ZAMT055 / ZAMT003\`
       \`지급수수료 원단위 = ZAMT040 / 판매중량(kg)\`
     조회된 metric.formula 에 의존 산식이 있으면 마지막 줄에 함께 표기.

  5) **분석질문 안내(선택)**: 마지막 줄에 괄호로 안내
     "(분석질문을 이용하면 실제 상관계수와 상세 분석 결과를 확인할 수 있습니다.)"

절대 지켜야 할 규칙:
- "결과값을 확정할 수 없습니다", "처리할 수 없습니다" 같은 부정·차단 표현 금지
- 상관계수 값을 임의로 만들어 제시하지 말 것 (실제 계산을 수행하지 않았음)
- 마크다운 표/차트 삽입 금지. 산식은 백틱 코드 인라인만 사용
- SQL 문장 삽입 금지 (현황집계 handler 는 SQL 실행하지 않음)`
    : '';
...
- SQL 문장을 답변에 넣지 마세요 (이 handler 는 SQL 실행 경로가 아님)
