...
def fetch_report_data(calmonth: str) -> dict:
    ...
    try:
        ...
        # 해당 월 존재 확인
        cur.execute("SELECT COUNT(*) as cnt FROM bw_profitability_data WHERE CALMONTH=%s", (calmonth,))
        row = cur.fetchone()
        ...
        cur.execute(f"""
            ...
            FROM bw_profitability_data WHERE CALMONTH=%s
        """, (calmonth,))
...
def generate_ppt(calmonth: str) -> io.BytesIO:
    ...
    _add_textbox(slide1, Inches(1.5), Inches(4.6), Inches(8), Inches(0.4),
                 f'총 거래건수: {data["total_rows"]:,}건  |  데이터 기준: bw_profitability_data',
...
def generate_ppt_with_prompt(calmonth: str, prompt: str = '', attachment_info: dict = None) -> io.BytesIO:
    ...
    info_text = f'총 거래건수: {data["total_rows"]:,}건  |  데이터 기준: bw_profitability_data'
    if prompt:
        info_text += f'\n분석 요청: {prompt[:80]}'
