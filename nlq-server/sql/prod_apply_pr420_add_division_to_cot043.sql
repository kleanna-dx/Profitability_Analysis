-- ============================================================
-- [운영 반영용 통합 스크립트] PR #420
-- sys_aimd_cot043 에 DIVISION / DIVISION_NM 컬럼 추가 + Ontology 시드
--
-- 작성일  : 2026-09-04
-- 대상 PR :
--   - PR #420 : sys_aimd_cot043 에 제품군 코드/명칭 컬럼 추가
--               원본 마이그레이션 파일 (jswon → develop 반영본):
--                 · sql/053_add_division_to_sys_aimd_cot043.sql (DDL, 신규)
--                 · sql/050_seed_cost_ontology.sql              (ontology 시드, PR #420 델타 6행)
--                 · sql/047_create_sys_aimd_cot043.sql          (원본 CREATE 정의 갱신 — 신규 환경용)
--
-- 이전 PR과의 관계:
--   PR #415~#419 에서 sys_aimd_cot015 에 동일 컬럼 (DIVISION VARCHAR(2), DIVISION_NM VARCHAR(40))
--   을 이미 추가함. 본 PR 은 sys_aimd_cot043 (원가요소별 금액, NLP_RFC_003 / Z_BI_WEB_EX_BL_5)
--   에도 동일 컨벤션으로 추가.
--
-- 실행 방법:
--   mysql -u <user> -p <database> < prod_apply_pr420_add_division_to_cot043.sql
--   또는
--   mysql> USE <database>;
--   mysql> SOURCE prod_apply_pr420_add_division_to_cot043.sql;
--
-- ⚠️ 사전 조건:
--   1) 아래 테이블이 운영 DB 에 이미 존재해야 합니다:
--        · sys_aimd_cot043 (원가요소별 금액) ← sql/047_create_sys_aimd_cot043.sql
...
--      (SapRfcSyncService · NLP_RFC_003 → Z_BI_WEB_EX_BL_5) 의 SAP → DB
...
--
-- ▣ 본 스크립트가 수행하는 작업 (요약)
--   1) sys_aimd_cot043 에 DIVISION     VARCHAR(2)  NULL AFTER COSTELMNT_NM  (SAP CHAR 2)
--   2) sys_aimd_cot043 에 DIVISION_NM  VARCHAR(40) NULL AFTER DIVISION       (SAP CHAR 40)
--   3) ontology_column 에 3도메인 (PS/HL/MGMT) × 2컬럼 = 6행 시드
--
-- 스키마 변경: YES (DDL 추가)
-- 데이터 변경: YES (ontology_column 6행 추가)
-- 서버 코드   : YES (SapRfcSyncService.DB_COLUMNS_COT043 확장 — module-profit 재배포 필요)
-- 환경변수    : 없음
-- ============================================================
...
SELECT
  CASE WHEN (SELECT COUNT(*) FROM INFORMATION_SCHEMA.TABLES
              WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'sys_aimd_cot043') = 1
       THEN '✓ sys_aimd_cot043 존재'
       ELSE '✗ sys_aimd_cot043 없음 — sql/047_create_sys_aimd_cot043.sql 을 먼저 실행하세요'
  END AS check_table_cot043,
  CASE WHEN (SELECT COUNT(*) FROM INFORMATION_SCHEMA.TABLES
...
-- ═══════════════════════════════════════════════════════════
-- 1) sys_aimd_cot043 에 DIVISION 컬럼 추가 (없을 때만)
--    타입: VARCHAR(2) — SAP RFC 원본 정의 CHAR 2 그대로
--    위치: AFTER COSTELMNT_NM
--    코멘트: '제품군'
-- ═══════════════════════════════════════════════════════════
SET @col_exists := (
...
-- ═══════════════════════════════════════════════════════════
-- 2) sys_aimd_cot043 에 DIVISION_NM 컬럼 추가 (없을 때만)
...
-- ------------------------------------------------------------
-- 1) 컬럼 존재 및 순서 확인
--    (COSTELMNT_NM → DIVISION → DIVISION_NM → COSTCENTER 순서여야 함)
...
--
-- 2) sys_aimd_cot043 총 컬럼 개수 (기대: 12개 = seq + 9개 원본 + DIVISION/DIVISION_NM)
...
--     WHERE table_name = 'sys_aimd_cot043'
...
-- ------------------------------------------------------------
-- (1) module-profit 재배포 (필수)
--     · Spring Boot SapRfcSyncService.DB_COLUMNS_COT043 에 DIVISION/DIVISION_NM 이 추가되었음
--     · 재기동 없이는 RFC 실행해도 두 컬럼이 매핑되지 않아 NULL 만 채워짐.
--
-- (2) RAG 인덱스 재빌드 (필수)
--     · 관리자 세션으로 학습관리 화면 접속 → [RAG 인덱스 재빌드] 버튼
--     · 또는 API 호출: POST /api/rag/build
--     → rag_embeddings 에 sys_aimd_cot043.DIVISION / DIVISION_NM 청크가 등록되어야
--        LLM 이 원가요소별 금액에서도 제품군 필터를 인식/생성 가능.
--
-- (3) SAP BW 어댑터 확인 (실제 값 채우기)
--     · Spring Boot SapRfcSyncService 의 NLP_RFC_003 (Z_BI_WEB_EX_BL_5)
--       매핑에 DIVISION / DIVISION_NM 이 추가되어 있는지 확인 (본 PR 에 포함).
--     · 다음 RFC 재적재 스케줄부터 값이 채워짐.
--
-- (4) 스모크 테스트 (권장)
--     · 자연어질의: "HL 제품군의 원가 요소별 금액 알려줘"
--       → SQL 이 WHERE DIVISION = 'HL' 로 sys_aimd_cot043 를 조회하는지 확인.
--     · 자연어질의: "PS 제품군 재료비 코스트센터별 합계"
