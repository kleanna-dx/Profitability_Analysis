...
-- ※ domain_code 컬럼으로 도메인(영역)별 데이터 분리
-- ============================================================

-- 1) 온톨로지 컬럼 사전
CREATE TABLE IF NOT EXISTS `ontology_column` (
  `id`          int(11)      NOT NULL AUTO_INCREMENT,
  `domain_code` varchar(20)  DEFAULT 'PS'  COMMENT '영역 코드 (PS, HL, MGMT)',
  ...
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_col_table` (`domain_code`, `column_name`, `table_name`)
...
CREATE TABLE IF NOT EXISTS `metric` (
  ...
  `updated_at`  datetime     DEFAULT current_timestamp() ON UPDATE current_timestamp(),
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_metric_code` (`domain_code`, `metric_code`)
...
CREATE TABLE IF NOT EXISTS `code_mapping` (
  ...
  `updated_at`     datetime     DEFAULT current_timestamp() ON UPDATE current_timestamp(),
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_col_code` (`domain_code`, `column_name`, `code_value`, `table_name`),
